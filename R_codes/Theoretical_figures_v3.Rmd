---
title:  'Code for theoretical figures'
subtitle: 'The interplay between migration and selection on the dynamics of pathogen variants'
output: pdf_document
---
...
# Fig. 3. Migration can interfere with selection and change the local dynamics of the frequency of an emerging variant
...
plot_grid(
  ...
  plot_grid(
    ...
    tab2 %>%
      ...
      scale_fill_manual(values=c(colors[2],'#99c2e6'), labels=c("S^B", "H^B")) +
      theme_classic() +
...
# ggsave("Fig3.png", width=8, height=4, dpi=1000)
```

# Fig. S3. Migration can steepen the change of local frequency of an emerging variant
...
plot_grid(
  ...
  subset(simul[seq(1,nrow(simul),4),], Approx_WM==FALSE & time<=tf2) %>%
    ...
    scale_y_continuous(expand=c(0,0), limits=c(logit(q0),7), breaks=seq(-5,5,2.5)) +
    theme_classic() +
    annotate(geom='text', label="No migration", x=-65, y=3.75,
             hjust=0, col='grey60', fontface='italic', size=3) +
...
# Fig. S2. Evolutionary dynamics of an emerging variant in a nine-patch host metapopulation
...
# Fig. S4. The effect of population size asymmetry on the dynamics of the variant frequency in the focal population.
...
# Fig. S6. The transient disruption of spatial differentiation
...
simul <- nratio_vec %>% lapply(function(nratio){
  ...
  return(simulate(init, times=seq(0,1100,1), parms=parms) %>% cbind('n_ratio'=nratio))
}) %>% bind_rows
...
# Fig. S5. Short-term dynamics of spatial differentiation and the migration–selection balance
...
## E-F (higher levels of migration)
...
# ggsave(width=9, height=10, dpi=600, 'FigS5.png')
```

# Fig. S7. Long-term spatial differentiation and local adaptation
...
# Fig. S10. The effect of reduced mobility in infected hosts on the evolutionary dynamics
...
(Fig_zeta1 <- ggarrange(
  ...
  ncol=2, labels=c("A", "B"), common.legend=TRUE, legend="top")) # 3.5 x 6
```
...
```{r}
plot_grid(Fig_zeta1, Fig_zeta2, labels=c("", "C"), ncol=1)
```